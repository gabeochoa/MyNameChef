...
namespace {
std::string get_opponent_match_server_url() {
...
TEST(validate_server_opponent_match) {
  ...
  setup_complete = setup_complete || []() {
    log_info("OPPONENT_MATCH_TEST: Setting up BattleLoadRequest with server URL...");

    std::string server_url = get_opponent_match_server_url();
    log_info("OPPONENT_MATCH_TEST: Using server URL: {}", server_url);
    ...
    log_info("OPPONENT_MATCH_TEST: BattleLoadRequest configured with server URL");
    return true;
  }();
  ...
  app.expect_true(std::filesystem::exists(req.opponentJsonPath),
                  "opponent JSON file should exist");
  
  std::ifstream file(req.opponentJsonPath);
  app.expect_true(file.is_open(), "opponent JSON file should be readable");
  ...
  for (const auto &dish_entry : team) {
    ...
    app.expect_true(dishTypeOpt.has_value(),
                    std::string("dishType should be valid enum: ") + dishTypeStr);
  ...
  for (afterhours::Entity &entity :
    ...
    log_info("OPPONENT_MATCH_TEST: Found dish at slot {}: {}",
             slot, magic_enum::enum_name(dish.type));
  }
  ...
  for (const auto &[slot, expected_type] : expected_dishes) {
    ...
    app.expect_eq(static_cast<int>(actual_type), static_cast<int>(expected_type),
                  std::string("opponent dish at slot ") +
                      std::to_string(slot) + " should match JSON");
    
    log_info("OPPONENT_MATCH_TEST: ✅ Slot {} matches: {}",
             slot, magic_enum::enum_name(expected_type));
  }
