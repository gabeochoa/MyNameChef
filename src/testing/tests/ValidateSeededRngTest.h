...
TEST(validate_seeded_rng_determinism) {
  ...
  app.expect_count_eq(static_cast<int>(first_run.size()), SHOP_SLOTS, "shop items from first run");
  ...
  app.expect_count_eq(static_cast<int>(second_run.size()), SHOP_SLOTS, "shop items from second run");

  // Verify both runs produced identical results
  app.expect_count_eq(static_cast<int>(first_run.size()), static_cast<int>(second_run.size()), 
                      "shop item count matches between runs");
  
  for (size_t i = 0; i < first_run.size(); ++i) {
    app.expect_eq(static_cast<int>(first_run[i]), static_cast<int>(second_run[i]),
  ...
  // Verify different seed produced different results (at least one item differs)
  bool found_difference = false;
  for (size_t i = 0; i < std::min(first_run.size(), different_seed_run.size()); ++i) {
    if (first_run[i] != different_seed_run[i]) {
      found_difference = true;
      break;
    }
  }
  app.expect_true(found_difference, "different seed produces different shop items");
}
...
TEST(validate_seeded_rng_helper_methods) {
  ...
  app.expect_eq(static_cast<int>(idx1), static_cast<int>(idx1_again), "gen_index is deterministic");
