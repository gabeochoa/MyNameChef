...
struct ValidateDebugDishTest {
  static void execute() {
    log_info("DEBUG_DISH_TEST: Starting DebugDish validation");

    test_debug_dish_creation();
    test_debug_dish_onserve_flavor_stats();
    test_debug_dish_onserve_target_scopes();
    test_debug_dish_onserve_combat_mods();
    test_debug_dish_onstartbattle();
    test_debug_dish_oncoursestart();
    test_debug_dish_onbitetaken();
    test_debug_dish_ondishfinished();
    test_debug_dish_oncoursecomplete();

    log_info("DEBUG_DISH_TEST: All tests completed");
  }
  ...
  // individual effects.
  static void test_debug_dish_creation() {
    log_info(
        "DEBUG_DISH_TEST: Testing DebugDish creation and basic properties");
  ...
  static void test_debug_dish_onserve_flavor_stats() {
  ...
  static void test_debug_dish_onserve_target_scopes() {
    ...
    auto &opponent =
        add_dish_to_menu(DishType::Potato, DishBattleState::TeamSide::Opponent,
                         1, DishBattleState::Phase::InCombat, true);
    ...
    if (!future_ally.has<PendingCombatMods>()) {
      log_error(
          "DEBUG_DISH_TEST: FutureAllies target missing PendingCombatMods");
      valid = false;
    }
  ...
  static void test_debug_dish_onserve_combat_mods() {
    ...
    if (self_mods.bodyDelta != 2) {
      log_error("DEBUG_DISH_TEST: Self bodyDelta wrong: {}",
                self_mods.bodyDelta);
      return;
    }
  ...
  // begins.
  static void test_debug_dish_onstartbattle() {
    ...
    if (!debug_dish.has<PendingCombatMods>()) {
      log_error(
          "DEBUG_DISH_TEST: OnStartBattle Self missing PendingCombatMods");
      return;
    }

    if (!ally.has<PendingCombatMods>()) {
      log_error(
          "DEBUG_DISH_TEST: OnStartBattle Ally missing PendingCombatMods");
      return;
    }
  ...
  // progression.
  static void test_debug_dish_oncoursestart() {
    ...
    if (!debug_dish.has<PendingCombatMods>()) {
      log_error(
          "DEBUG_DISH_TEST: OnCourseStart Self missing PendingCombatMods");
      return;
    }

    if (!ally.has<PendingCombatMods>()) {
      log_error(
          "DEBUG_DISH_TEST: OnCourseStart Ally missing PendingCombatMods");
      return;
    }
  ...
  static void test_debug_dish_onbitetaken() {
  ...
  // dish finishes.
  static void test_debug_dish_ondishfinished() {
    ...
    if (!ally.has<PendingCombatMods>()) {
      log_error(
          "DEBUG_DISH_TEST: OnDishFinished Ally missing PendingCombatMods");
      return;
    }
  ...
  static void test_debug_dish_oncoursecomplete() {
