...
TEST(validate_entity_query_without_manual_merge) {
  ...
  app.expect_eq(dish.get<IsDish>().type, DishType::Potato, "dish type is Potato");
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    e.addComponent<Transform>(vec2{100.0f + i * 100.0f, 100.0f}, vec2{80.0f, 80.0f});
  ...
  for (Entity &entity : EQ().whereHasComponent<IsDish>().gen()) {
    if (std::find(created_ids.begin(), created_ids.end(), entity.id) != created_ids.end()) {
      found_count++;
    }
  }
  
  app.expect_eq(found_count, 3, "all 3 created entities found after one frame");
  log_info("VALIDATION_TEST: Test 2 PASSED - Entities queryable after system loop");

  // Test 3: Battle Team Entities Queryable After System Loop
  log_info("VALIDATION_TEST: Test 3 - Battle team entities queryable after system loop");
  ...
  auto &cq_entity = EntityHelper::get_singleton<CombatQueue>();
  app.expect_false(cq_entity.get().has<BattleTeamDataPlayer>(), "BattleTeamDataPlayer should not exist yet");
  BattleTeamDataPlayer player_data;
  player_data.team.push_back({DishType::Potato, 0, 1});
  player_data.instantiated = false;
  cq_entity.get().addComponent<BattleTeamDataPlayer>(std::move(player_data));
  
  app.expect_false(cq_entity.get().has<BattleTeamDataOpponent>(), "BattleTeamDataOpponent should not exist yet");
  BattleTeamDataOpponent opponent_data;
  opponent_data.team.push_back({DishType::Potato, 0, 1});
  opponent_data.instantiated = false;
  cq_entity.get().addComponent<BattleTeamDataOpponent>(std::move(opponent_data));
  ...
  app.expect_true(opponent_dishes > 0, "opponent dishes found after system loop");
  log_info("VALIDATION_TEST: Test 3 PASSED - Battle team entities queryable");

  // Test 4: Inventory Slots Queryable After Generation
  log_info("VALIDATION_TEST: Test 4 - Inventory slots queryable after generation");
  
  // Navigate to shop screen (triggers GenerateInventorySlots)
  GameStateManager::get().to_shop();
  GameStateManager::get().update_screen();
  ...
  auto found_opt = EQ({.force_merge = true}).whereID(force_merge_test_id).gen_first();
  app.expect_true(found_opt.has_value(), "entity found with force_merge");
  app.expect_eq(found_opt.asE().get<IsDish>().type, DishType::Salmon, "entity type matches");
