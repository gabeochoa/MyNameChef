...
struct RenderBattleTeams : afterhours::System<Transform, IsDish> {
  ...
  void for_each_with(const afterhours::Entity &entity, const Transform &transform, 
                     const IsDish &dish, float) override {
    ...
    float labelX = transform.position.x + (transform.size.x - labelWidth) / 2.0f;
