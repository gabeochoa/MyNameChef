...
if [ "${AUTO_TEST:-false}" = "true" ]; then
    ...
    if timeout 60 "$CLIENT_EXECUTABLE" --run-test validate_server_battle_integration --headless --timing-speed-scale 5 > /tmp/integration_client_$$.log 2>&1; then
    ...
    "$CLIENT_EXECUTABLE" --run-test validate_server_battle_integration --timing-speed-scale 5
    TEST_EXIT_CODE=$?
    ...
    "$CLIENT_EXECUTABLE" --run-test validate_server_opponent_match --timing-speed-scale 5
